...
async function displayAllText() {
    try {
        ...
        console.log('Total number of strings:', totalStrings);

        let allStrings = '';
        for (let i = 0; i < totalStrings; i++) {
            const currentString = await contract.getString(i);
            allStrings += `String ${i + 1}: ${currentString}\n`;
        }

        document.getElementById('allStringsDisplay').innerText = allStrings;
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    document.getElementById('displayAllStringsButton').addEventListener('click', displayAllText);
});
